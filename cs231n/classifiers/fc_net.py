...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params.update({
            "W{}".format(i + 1):weight_scale * np.random.randn(dims[i], dims[i + 1])
            for i in range(len(dims) - 1)
        })
        self.params.update({
            "b{}".format(i + 1):np.zeros(dims[i + 1])
            for i in range(len(dims) - 1)
        })
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        if self.normalization=='batchnorm':
            ...
            self.params.update({
                "beta{}".format(i + 1):np.zeros(dims[i + 1])
                for i in range(len(dims) - 2)
            })
            self.params.update({
                "gamma{}".format(i + 1):np.ones(dims[i + 1])
                for i in range(len(dims) - 2)
            })
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if self.normalization == "batchnorm":
                scores, cache = affine_bn_relu_forward(scores, self.params["W{}".format(i + 1)],
                    self.params["b{}".format(i + 1)], self.params["gamma{}".format(i + 1)],
                    self.params["beta{}".format(i + 1)], self.bn_params[i])
            else:
                scores, cache = affine_relu_forward(scores, self.params["W{}".format(i + 1)],
                    self.params["b{}".format(i + 1)])
            cache_history.append(cache)
            if self.use_dropout:
                scores, cache = dropout_forward(scores, self.dropout_param)
                cache_history.append(cache)
            l2_reg += np.sum(self.params["W{}".format(i + 1)] ** 2)
        scores, cache = affine_forward(scores, self.params["W{}".format(self.num_layers)],
            self.params["b{}".format(self.num_layers)])
        cache_history.append(cache)
        l2_reg += np.sum(self.params["W{}".format(self.num_layers)] ** 2)
        l2_reg *= 0.5 * self.reg
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, grads["W{}".format(self.num_layers)], grads["b{}".format(self.num_layers)] =\
            affine_backward(dout, cache_history.pop())
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.normalization == "batchnorm":
                dout, grads["W{}".format(i)], grads["b{}".format(i)], grads["gamma{}".format(i)], grads["beta{}".format(i)] =\
                affine_bn_relu_backward(dout, cache_history.pop())
            else:
                dout, grads["W{}".format(i)], grads["b{}".format(i)] =\
                affine_relu_backward(dout, cache_history.pop())
