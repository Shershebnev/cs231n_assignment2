...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N = dout.shape[0]
    xhat, mu, var, eps, gamma, beta, x = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * xhat, axis=0)
    dxhat = dout * gamma
    dxmu1 = dxhat * 1 / np.sqrt(var + eps)
    divar = np.sum(dxhat * (x - mu), axis=0)
    dvar = divar * -0.5 * (var + eps) ** (-1.5)
    dsq = 1 / N * np.ones_like(dout) * dvar
    dxmu2 = 2 * (x - mu) * dsq
    dx1 = dxmu1 + dxmu2
    dmu = -np.sum(dxmu1 + dxmu2, axis=0)
    dx2 = 1 / N * np.ones_like(dout) * dmu
    dx = dx1 + dx2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_trans = x.T
    N = x_trans.shape[0]
    mu = np.mean(x_trans, axis=0)
    var = 1 / N * np.sum((x_trans - mu) ** 2, axis=0)
    x_hat = (x_trans - mu) / np.sqrt(var + eps)
    x_hat = x_hat.T
    out = gamma * x_hat + beta
    cache = (x_hat, mu, var, eps, gamma, beta, x_trans)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    divar = np.sum(dxhat * (x - mu), axis=0)
    dvar = divar * -0.5 * (var + eps) ** (-1.5)
    dsq = 1 / N * np.ones_like(dout.T) * dvar
    dxmu2 = 2 * (x - mu) * dsq
    dx1 = dxmu1 + dxmu2
    dmu = -np.sum(dxmu1 + dxmu2, axis=0)
    dx2 = 1 / N * np.ones_like(dout.T) * dmu
    dx = dx1 + dx2
    dx = dx.T
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(F):
        for c in range(C):
            for hh in range(HH):
                for ww in range(WW):
                    x_padded_slice = x_padded[:, c, hh:hh + H_out * stride:stride, ww:ww + W_out * stride:stride]
                    dw[f, c, hh, ww] = np.sum(dout[:, f, :, :] * x_padded_slice)
                ...
                for f in range(F):
                    for h_out in range(H_out):
                        for w_out in range(W_out):
                            ...
                            if hx + pad - h_out * stride < HH and hx + pad - h_out * stride >= 0:
                                mask1[:, hx + pad - h_out * stride, :] = 1.0
                            if wx + pad - w_out * stride < WW and wx + pad - w_out * stride >= 0:
                                mask2[:, :, wx + pad - w_out * stride] = 1.0
                            w_masked = np.sum(w[f, ...] * mask1 * mask2, axis=(1, 2))
                            dx[n, :, hx, wx] += dout[n, f, h_out, w_out] * w_masked
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def spatial_groupnorm_backward(dout, cache):
    ...
    N_hat = dxhat.shape[0]
    dx = 1 / N_hat * ivar * (N_hat * dxhat - np.sum(dxhat, axis=0) - xhat * np.sum(dxhat * xhat, axis=0))
